...
namespace QueueLab
{
    ...
    class Program
    {
        ...
        private static int totalOperations = 0; // Track total operations performed
        private static DateTime sessionStart = DateTime.Now; // Track session start time
        ...
        static void HandleProcessTicket()
        {
            ...
            totalOperations++;
            Console.WriteLine($"✅ Processing Ticket: {nextTicket.TicketId} - {nextTicket.ToDetailedString()}");
            if (ticketQueue.Count > 0)
            {
                SupportTicket upcomingTicket = ticketQueue.Peek();
                Console.WriteLine($"👀 Next ticket in queue: {upcomingTicket.TicketId} - {upcomingTicket.Description}\n");
            }
            else
            {
        ...
        static void HandlePeekNext()
        {
            ...
            Console.WriteLine($"✅ Next Ticket: {nextTicket.TicketId} - {nextTicket.ToDetailedString()}");
        ...
        static void HandleDisplayQueue()
        {
            ...
            {
                Console.WriteLine($"{position:D2}. {ticket} {(position == 1 ? "← Next" : "")}");
                position++;
            }
        }
        ...
        static void HandleClearQueue()
        {
            ...
            Console.Write($"This will remove {countToClear} tickets. Are you sure? (y/N): ");
            string response = Console.ReadLine()?.ToLower() ?? "";
            if (response == "y" || response == "yes")
            {
                Console.WriteLine("✅ Clearing all tickets...");
                ticketQueue.Clear();
                totalOperations++;
        ...
        static void HandleUrgentTicket()
        {
            ...
            Console.WriteLine("⚠️ Urgent tickets are processed first!\n");
